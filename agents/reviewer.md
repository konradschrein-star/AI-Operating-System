...
- Vault writes go under the agent root only: when `VAULT_LAYOUT=split`, everything an agent writes lives under `Forge/` and Konrad's side (`Konrad/`) is read-only for agents — an agent-actor write aimed at his side is a finding, not a style note (`forge-control/src/lib/vault-layout.ts`).
