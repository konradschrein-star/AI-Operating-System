...
- **Nginx SSE** at `/etc/nginx/sites-enabled/os.schreinercontentsystems.com` — must set `proxy_buffering off; proxy_read_timeout 1h;` for `/api/proxy/chat/*/stream`. Verify before shipping.
