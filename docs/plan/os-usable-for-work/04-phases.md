...
| round 13, `fix(preflight C5): …` | `scripts/checks/preflight-deploy.sh` | **nobody** — built by task `4a645d9d` at round 0 of this same `main` workstream | Check C5 went **PASS → FAIL on the vault merge**, and the checker was wrong twice, not the code. (a) B1c replaced `noteCounts`'s `Promise<Record<NoteCategory \| "all", number>>` with `Promise<MemoryCounts>` — that IS R15, and removing the bare `all` is what fixed "it's zero and not eight"; C5 knew one shape and correctly refused to guess. (b) `extractAccessedFields` did not strip comments, and phase 2 documented the fixed defect *inside* `MemorySurface.tsx`, so C5 read `counts.all` out of a JSX comment. **This is the gate that judges this task's own output**, so: the property was verified by an independent fresh comparator BEFORE the checker was touched, both produced the same two field sets, the inline self-test is untouched, and two forced failures (a field removed from the interface; a genuine bogus access in real code) are demonstrated firing. Detail and transcripts in `phase7/integration-report.md` §7. |
