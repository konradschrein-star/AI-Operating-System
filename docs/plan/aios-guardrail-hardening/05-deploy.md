...
5. `python3 scripts/ops/test-guard-autonomy.py` on the deployed checkout: 188/188
   and, critically, **zero new `guardrail_trips` rows** — Layer A is in-process
...
6. `bash scripts/checks/prove-guard-bites.sh` → `BITES`, 7/7 mutations
   discriminated, and the hook's md5 unchanged across the run.
