...
# Round 242 — E4 reopened: the fact is recorded, and the straddle closes

Round 223 ruled option B and named the one change that would make option A
sound. This is that change, taken inside the window 223 priced — *cheap only
while 0040 is un-applied, which is true until phase 8 runs `psql -f`*.
**§5 below is not superseded as an argument. Its four failed gates are the
evidence for the column, and its arms still RUN**, against the pre-242 rows, as
`PRE-E4` and `WIDE-*` in the same script.

Requirements touched: **R71** (new), **R69** (widened), **R6** and §3.2's
sentence (restored), **F14** (retired). Files written:
`db/migrations/0040_task_graph.sql`, `forge-control/src/db/projects.ts`,
...
`forge-control/src/lib/project-tick.ts` (the F-E ruling — one constant),
...
`forge-control/src/routes/chat.ts`, `scripts/checks/check-r69-straddle.sh`,
`scripts/checks/check-scheduler-sql.sh`,
`scripts/checks/check-migration-0040.sh`, the three corpus documents, and this
section. `evidence/phase2-replay.md`'s generated census region was regenerated
by its own script (`--write`), not hand-edited.

## 11. The change, and the two transcripts it is proved by

### 11.1 The migration diff — one statement, one clause
...
**Same statement, same `WHERE`, same transaction.** There is no interleaving in
which a row carries a backfilled closure and a false marker, or the reverse,
because one statement writes both — which is the whole difference between a
recorded fact and an inferred one. `DEFAULT false` is what makes the column
additive: every row any engine ever wrote, before or after 0040 runs, is
correctly not-frozen without being visited.

R69's predicate, in the pure module and mirrored in the SQL:

```
 ready = every dep is done
-        AND NOT ∃ o : o.depends_on === null ∧ o.round < task.round ∧ o.status !== 'done'
+        AND NOT ∃ o : o.round < task.round ∧ o.status !== 'done'
+                      ∧ (task.graph_frozen ∨ o.depends_on === null)
```

The disjunct is keyed on the **candidate's** provenance, not the blocker's, and
that is the whole repair: a closure the migration derived is untrustworthy
against every later row, whoever wrote it.

### 11.2 The straddle, both ways — `check-r69-straddle.sh`, 14 probes, all green

Eight arms now. `MARKED` is the engine as it ships; `PRE-E4` is the same SHIPPED
rule over the same rows with the marker cleared (round 223's engine); `NO-TERM`
is the shipped rule with R69's `graph_frozen` disjunct **deleted from a copy of
the module**, generated by the script, which refuses unless the deletion matches
exactly once:

```
  mutant diff vs task-graph.ts (the ONE line the experiment deletes):
    375c375
    <     if (frozen || other.depends_on === null) return false;
    ---
    >     if (other.depends_on === null) return false;   // MUTANT: graph_frozen term deleted
```

```
  S1 — straddle, NO gap row — 131 frozen rows + a post-restart fix chain at 1307/1308
      LEGACY         ticks= 17 promoted= 10 widest-tick= 2
      MARKED         ticks= 17 promoted= 10 widest-tick= 2
      NO-TERM        ticks= 14 promoted= 10 widest-tick= 2
      PRE-E4         ticks= 14 promoted= 10 widest-tick= 2
      WIDE-SENTINEL  ticks= 14 promoted= 10 widest-tick= 2   widening: evaluated=52 gate-open=0 fired=0
      WIDE-CLOSURE   ticks= 14 promoted= 10 widest-tick= 2   widening: evaluated=52 gate-open=0 fired=0
      WIDE-HORIZON   ticks= 17 promoted= 10 widest-tick= 2   widening: evaluated=76 gate-open=72 fired=6
      WIDE-UNGATED   ticks= 17 promoted= 10 widest-tick= 2   widening: evaluated=76 gate-open=76 fired=6
  S3 — post-restart project — 1 planner, 1 long reviewer, 7 unrelated builders numbered above it
      LEGACY         ticks= 17 promoted=  8 widest-tick= 1
      MARKED         ticks=  3 promoted=  8 widest-tick= 8
      NO-TERM        ticks=  3 promoted=  8 widest-tick= 8
      PRE-E4         ticks=  3 promoted=  8 widest-tick= 8
      WIDE-SENTINEL  ticks=  3 promoted=  8 widest-tick= 8   widening: evaluated=8 gate-open=0 fired=0
      WIDE-CLOSURE   ticks=  3 promoted=  8 widest-tick= 8   widening: evaluated=8 gate-open=1 fired=0
      WIDE-HORIZON   ticks= 17 promoted=  8 widest-tick= 1   widening: evaluated=64 gate-open=64 fired=56
      WIDE-UNGATED   ticks= 17 promoted=  8 widest-tick= 1   widening: evaluated=64 gate-open=64 fired=56

  ok   1  S1: WITHOUT the marker the engine diverges from today's — both ways of removing it
         marker CLEARED (the pre-E4 schema): tick 2: only-first []; only-second [511070c9, 608dbecb]
         term DELETED from the source:       tick 2: only-first []; only-second [511070c9, 608dbecb]
         legacy ticks=17, without-the-marker ticks=14
  ok   1b  S1: WITH the marker the SHIPPED engine matches today's engine tick for tick (E4)
         legacy      ticks=17 promoted=10 widest-tick=2
         shipped     ticks=17 promoted=10 widest-tick=2
         first divergence: NONE — identical, tick for tick
  ok   1c  the two removals are the SAME removal — clearing the fact equals deleting the term
         S1: marker-cleared vs term-deleted — identical, tick for tick
         S2: marker-cleared vs term-deleted — identical, tick for tick
         S3: marker-cleared vs term-deleted — identical, tick for tick
         S1: shipped vs term-deleted — tick 2: only-first []; only-second [511070c9, 608dbecb]
  ok   3b  S3: the MARKER costs a post-restart project NOTHING — the price E4 does not pay
         S3 holds 0 frozen rows, so the marker term can never fire here.
           pre-E4 (the engine round 223 measured): ticks=3 widest-tick=8
           shipped, with the marker:               ticks=3 widest-tick=8   ← identical, tick for tick
           the same widening, ungated:             ticks=17 widest-tick=1   ← the collapse E4 had to avoid

  registered 14   ran 14   passed 14   failed 0   never-ran 0
```

**Read the three readings together.** 17 ticks with the marker on the straddle —
today's engine exactly, first divergence GONE, on the fixture whose divergence
§3.2.1 pinned at tick 2 on `511070c9…` and `608dbecb…`. Those two ids come back,
on that tick, under BOTH removals. And 3 ticks / 8-wide on a project planned
after the restart, byte-identical to the engine before the column existed —
which is what every inferred gate could not do: `WIDE-HORIZON`, the cleverest of
them, fires 56 times there and serialises eight independent builders.

**What would have made this instrument report a pass wrongly, answered by the
instrument.** (i) *"Both arms ran the same code."* Probe 1c asserts the mutant
DISAGREES with the shipped arm on S1 — an arm that agreed everywhere would be no
mutant at all — and the generator refuses unless its deletion matched exactly
once. (ii) *"The fixture was not straddling."* Probe 0b still counts 8 frozen
`pending` rows above the chain with a closure that cannot name it, with
`graphReady()` answering for 8/8. (iii) *"The marker was set on everything, so of
course the straddle matches."* Probe 3b counts frozen rows on S3 and requires
**0** before it reads the schedules; `check-migration-0040.sh` asserts on a real
Postgres that the marker and the closure agree on every row and that a row
inserted AFTER the backfill is not frozen.

### 11.3 The replay proof — case (g), and its positive control

`task-graph-replay.test.ts` gains **case (g)**: the same fix chain as case (f),
at the same two rounds, created by the NEW engine so it carries real
`depends_on` (R42) instead of the NULL sentinel. Case (f) is the order R69
already closed; case (g) is the order F14 recorded as accepted.

```
task-graph-replay: CASE     R18-f fix chain inserted after the migration legacy 17t/10p/0w  graph 17t/10p/0w
task-graph-replay: CASE     R18-g fix chain created after the restart    legacy 17t/10p/0w  graph 17t/10p/0w
task-graph-replay: CONTROL  case (g) without graph_frozen — diverges at tick 2,
                            graph promoted early: [511070c9, 608dbecb] (graph 14 ticks vs legacy 17)
```

The two cases agreeing is itself a reading: **the schedule a straddling project
gets no longer depends on which side of the restart its fix chain was born on.**
The control is in the same file and clears the marker on the same input, so the
case cannot pass by accident of the snapshot — the failure it would otherwise
have is the one measured above, on the same tick, on the same two rows.

One instrument gate was **amended where it is enforced** (standing rule 2):
`graphInput()` withheld `round` when no row came out with `depends_on: null`,
which is a test for case (f)'s legacy rows. Case (g)'s rows carry real arrays, so
that condition would have called a straddling input pure, withheld its rounds,
and made R69's round-reading term unevaluable — the case would have been decided
by the harness rather than by the rule. The condition is now *"every row is
frozen"*, which is the property the withholding actually depends on; cases
(a)–(e) still withhold, so guard 3 is relocated, not weakened, and a test asserts
both directions plus case (g)'s.

### 11.4 The SQL layer — `check-scheduler-sql.sh` case 5b, against a real Postgres

93 assertions, all green, on a throwaway schema (`forge_tg_scratch`, schema
`tg_check_sched`) — never `content_forge`. Case 5b is case 5 with one difference:
the row at the strictly lower round is a GRAPH row the new engine wrote, not a
legacy one. **Two candidates, identical but for the marker**, is the control:

```
--- 5. case 5b — R69 widened: graph_frozen holds a DERIVED closure (E4) -------
  ok   E4 premise: the candidates share one done dependency     = done
  ok   E4 premise: the lower-round row is a GRAPH row, not a legacy one = f
  ok   E4 premise: that lower-round row is not done             = no
  ok   E4: the FROZEN candidate is NOT promoted                 = no
  ok   E4: the frozen candidate is still pending                = pending
  ok   E4 CONTROL: the DECLARED candidate promotes in the same tick = yes
  ok   E4 MIRROR: the frozen candidate took the graph branch    contains: MIRROR_RULE=graph
  ok   E4 MIRROR: graphReady() also refuses the frozen candidate contains: MIRROR=false
  ok   E4 MIRROR: the declared candidate took the graph branch  contains: MIRROR_RULE=graph
  ok   E4 MIRROR: graphReady() also releases the declared one   contains: MIRROR=true
  …
  ok   E4: the frozen candidate promotes once the graph row is done   = yes   (tick 2)
```

**The mirror is read BEFORE the promote, and the timing is the assertion's
content.** `graphReady()` answers `false` for any row that is not `pending`, so a
mirror taken after tick 1 would report `false` for the declared candidate
*because the statement had just promoted it* — an instrument agreeing with itself
about the wrong question. It is captured while both rows are `pending`, so the
pure side is asked exactly what the statement is about to answer. The driver's
projection SELECTs `graph_frozen`; without it the field would arrive `undefined`,
read as falsy inside the term, and the mirror would answer for a rule the engine
is not running.

`check-migration-0040.sh` grew seven assertions on the same scratch database (43
total, all green): the type, `NOT NULL DEFAULT false`, that all 131 backfilled
rows carry the marker, that **the marker and the closure agree on every row**
(`count(*) WHERE graph_frozen <> (depends_on IS NOT NULL)` = 0), that a row
inserted AFTER the backfill by an INSERT not naming the column is **not** frozen,
and that a second application marks nothing new. The snapshot the re-runnability
proof diffs now includes the column, so a marker that drifted on replay breaks
the byte-comparison.

### 11.5 F14 is retired, not annotated

§6's F14 row is struck through and its text replaced by the retirement, in this
commit, together with §3.2's restored sentence, §3.2.2's rewrite, R6's clause and
R69's narrowed wording (standing rule 4). It is kept as a struck row rather than
deleted because §3.2.2 and §9.3 both cite it and a reader who remembers the
accepted risk must be able to find where it was closed. **Nothing of it survives
as accepted risk.** Round 223's ruling in §9.3 is marked SUPERSEDED and moved
under §9.3.1 with its reasoning intact — it is the argument for the column, and
the four gates it measured still run as arms of the script.

### 11.6 The R20 census joins the universal gate (§3.1 item 8)

Round 223's other recommendation. It sat red across three rounds precisely
because it blocked nobody. At this round it exits 0 at **129 hits (51 code / 78
comment, 3 SQL annotations)** over `db/projects.ts` sha256 `79a62da97552c1c2…`;
the generated region in `evidence/phase2-replay.md` was regenerated with
`--write` in this commit, since this round's edits moved the comment counts.
**The code count did not move**: R69's new SQL clause reads `pt.graph_frozen`,
not `round`, so the scheduler gained no new `round` predicate and the R20
assertion passes unchanged.

**A rotted pin, reported rather than reinterpreted (standing rule 1).** This
task's brief states the census "currently exits 0 at 123 hits (51 code / 72
comment) over `db/projects.ts` sha256 `47e25793ae372be3…`". At `fe14a7e`, before
this round's first edit, it exited 0 at **124 hits (51 code / 73 comment) over
sha256 `f620b372a3076bbb…`**. The brief's pin describes an earlier tree; it is
recorded here as a finding and the live measurement is the one used.

### 11.7 F-E — the tester gets `BROWSER_FIRST`

Delivered in this commit as instructed: one constant appended to the tester
branch of `buildPrompt()`. **The derivation is untouched** — `withPolicy()` still
computes the browser-rule set from the body rather than a role list, and the
tester joins that set by carrying the constant, exactly as builder and scout do.
`EXPECTED_BROWSER_ROLES` in `project-tick.test.ts` becomes
`{builder, researcher, scout, tester}` and the complement test recomputes the
other four.

**The NF7 delta is zero, and it is measured rather than argued.** The maximal
planner prompt was built twice in one process — from `project-tick.ts` as it now
stands, and from a copy with this rider deleted:

```
with the rider  planner=12084  tester=7560  tester carries B4=true
without it      planner=12084  tester=5962  tester carries B4=false
```

NF7's gate measures the maximal PLANNER prompt, and the tester is a different
branch of the same function, so this change costs it **0 characters**. What it
costs the tester's own prompt is **+1598** — `BROWSER_FIRST` (731) +
`BROWSER_CONTROL_SAFETY` (863) + the two `\n\n` joins — and the fourth column of
that transcript is the delivery itself: the tester did not carry B4's run-control
rules before this commit and does now, through the derivation rather than through
a hand-written list.

**One pin corrected while checking it (standing rule 1).** The brief reserves
"176 characters of headroom against the 12237 cap", which is 5B's measurement at
its own tip. In this worktree the maximal planner prompt measures **12084**, so
the headroom is **153**. The 23-character difference is not this task's: it is
`GRAPH_GUIDE` growing in the concurrent round-242 fix-cycle commit `f135de4`
