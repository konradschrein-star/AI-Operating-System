...
*Generated by `scripts/checks/check-r20-census.py` from `forge-control/src/db/projects.ts`, sha256 `47e25793ae372be3…` — the region names the BYTES it was computed from, not a commit, so committing it does not make it stale. Do not hand-edit: `--write` regenerates it and the check gate fails when it disagrees with the file.*

**123 lines match `round`; 134 match case-insensitively.** Of the 123, **51 are code and 72 are comment prose** — and 3 of the code lines are SQL annotations, code whose only `round` sits after a trailing `--`. The 11 case-insensitive extras are identifier spellings (`listVerdictRound`, `VerdictRoundRow`, `legacyRoundReady`) and prose `ROUND`, covered by the same attributions.
