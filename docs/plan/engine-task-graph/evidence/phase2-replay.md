...
*Generated by `scripts/checks/check-r20-census.py` from `forge-control/src/db/projects.ts`, sha256 `3339022dee842c05…` — the region names the BYTES it was computed from, not a commit, so committing it does not make it stale. Do not hand-edit: `--write` regenerates it and the check gate fails when it disagrees with the file.*
