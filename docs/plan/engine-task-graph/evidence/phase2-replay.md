...
*Generated by `scripts/checks/check-r20-census.py` from `forge-control/src/db/projects.ts`, sha256 `79a62da97552c1c2…` — the region names the BYTES it was computed from, not a commit, so committing it does not make it stale. Do not hand-edit: `--write` regenerates it and the check gate fails when it disagrees with the file.*

**129 lines match `round`; 142 match case-insensitively.** Of the 129, **51 are code and 78 are comment prose** — and 3 of the code lines are SQL annotations, code whose only `round` sits after a trailing `--`. The 13 case-insensitive extras are identifier spellings (`listVerdictRound`, `VerdictRoundRow`, `legacyRoundReady`) and prose `ROUND`, covered by the same attributions.
...
| `promoteReadyTasks` | 14 | 4 | 10 | **The scheduler.** Its two predicate lines are R69's legacy-row term and R12's legacy branch, both `TODO(R12-retire)`; its other code hits are R27's project-correlation lines, whose `round` is in the trailing `--`. The graph branch reads no `round` at all — see the second table. |
