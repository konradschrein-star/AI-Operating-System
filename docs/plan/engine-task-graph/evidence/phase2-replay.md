...
*Generated by `scripts/checks/check-r20-census.py` from `forge-control/src/db/projects.ts`, sha256 `f620b372a3076bbb…` — the region names the BYTES it was computed from, not a commit, so committing it does not make it stale. Do not hand-edit: `--write` regenerates it and the check gate fails when it disagrees with the file.*

**124 lines match `round`; 135 match case-insensitively.** Of the 124, **51 are code and 73 are comment prose** — and 3 of the code lines are SQL annotations, code whose only `round` sits after a trailing `--`. The 11 case-insensitive extras are identifier spellings (`listVerdictRound`, `VerdictRoundRow`, `legacyRoundReady`) and prose `ROUND`, covered by the same attributions.
...
| `(module preamble)` | 7 | 0 | 7 | **The module header.** Corrected round 204: it now states the two-branch rule, the NULL sentinel, which branch survives only for rows the old engine wrote, what `round` is still for, and that a cardinality mismatch is corruption no route may turn into a run. |
