...
**Verified gap (narrowed round 1350):** the harness's **async task-completion notification** — the banner that announces a background agent finished — **never reaches the thread**. `cc-runner.ts:502–514` forwards only `tool_result` blocks from user events, with no `else` branch; the `CcEvent` union is closed (`init | assistant_text | tool_call | tool_result`, `:234–235`). `cc-runner.ts` is an off-limits engine file. Per the brief's escape hatch, phase 4 renders what exists and **documents this gap precisely** (deliverable `docs/plan/notification-gap.md`) for the engine-v2 lane. No new plumbing. *Old pins `:417–429` / `:170–188` drifted — they now point at the idle-timeout kill logic and `buildSystemPrompt`; corrected against `b02aa62`.*

**What this paragraph used to over-claim:** an earlier revision said agent completion payloads *never* reach `runs.thread` at all. That is **false**, and `notification-gap.md` §2 proves it: a **synchronous** sub-agent's final text arrives as a `tool_result` entry; an **async** sub-agent's own entries land inline tagged `meta.parent_tool_use_id` (196 of 285 entries in run `3853c154…`); and peer runs reach the thread through `POST /api/runs/:id/message` → `kind: "comms"`, already rendered as direction-marked cards. Only the completion *notification* is uncovered.
