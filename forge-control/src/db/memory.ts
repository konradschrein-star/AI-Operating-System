...
export async function syncVaultNotes(): Promise<VaultSyncResult> {
  ...
  for (const e of entries) {
    ...
    try {
      const raw = await readFile(abs, "utf8");
      // MERGE 2026-08-23: main refactored facts extraction into vaultNoteFacts(),
      // which is strictly better here — it routes drawings through the tested
      // extractDrawingText(), warns on a degraded graph, and unions the
      // plugin's own `excalidraw` frontmatter tag with the ones read off the
      // canvas. This lane's inline copy predated that helper.
      //
      // What the helper does NOT return is the rendered prose, and this lane
      // needs it: `searchableBody` is what gets embedded for a drawing further
      // down. So take main's facts and ask the same extractor for the text.
      const { topic, tags, wikilinks } = vaultNoteFacts(rel, raw);
      const searchableBody = isDrawing(rel) ? extractDrawingText(rel, raw).text : "";
