/**
 * Mentor accountability metrics (schema: 0028_telegram_coach.sql, table
 * renamed coach_metrics → mentor_metrics in 0029_mentor_rename.sql).
 *
 * The evening mentor run reports committed vs completed for the day (it
 * curls POST /api/mentor/metrics after reading the daily note's checkboxes).
...
pool.on("error", (e) => console.error("[mentor pool]", e.message));

export interface MentorDay {
  day: string;
  committed: number;
  completed: number;
  notes: string | null;
...
export async function upsertMentorDay(input: {
  day: string; // YYYY-MM-DD
  committed: number;
  completed: number;
  notes?: string;
}): Promise<MentorDay> {
  const r = await pool.query<MentorDay>(
    `INSERT INTO mentor_metrics (day, committed, completed, notes)
     VALUES ($1, $2, $3, $4)
     ON CONFLICT (day) DO UPDATE
       SET committed = EXCLUDED.committed,
           completed = EXCLUDED.completed,
           notes = COALESCE(EXCLUDED.notes, mentor_metrics.notes),
           updated_at = now()
...
export async function listMentorDays(limit = 30): Promise<MentorDay[]> {
  const r = await pool.query<MentorDay>(
...
function hitTarget(d: MentorDay): boolean {
...
export async function currentStreak(): Promise<number> {
  const days = await listMentorDays(120);
