...
export function MobileApp() {
  ...
  const resolveM = useMutation({
    mutationFn: ({ id, action_id }: { id: string; action_id?: string }) =>
      resolveInboxItem(id, { resolved_by: "user", action_id }),
    onSuccess: () => {
      ...
      >
        ...
        {tab === "inbox" && (
          <InboxScreen
            ...
            onResolve={(id, action_id) => resolveM.mutate({ id, action_id })}
          />
        )}
  ...
  return (
    <div className="slidein" style={{ padding: "18px 18px 28px" }}>
      ...
      {items.map((i) => {
        ...
        const open = !!inboxOpen[i.id];
        const isBleed = i.status === "BLEED";
        const hasTried = i.tried.length > 0;
        return (
          <div
            key={i.id}
            style={{
          ...
          >
            ...
            <div
              onClick={() => hasTried && onToggle(i.id)}
              style={{
                display: "flex",
                alignItems: "center",
                gap: 7,
            ...
            >
              <span
                className="ms"
                style={{ fontSize: 14, color: tokens.decide }}
              >
              ...
              >
                {hasTried
                  ? `Manager tried ${i.tried.length} things`
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 13 }}>
              {i.actions.map((a, j) => (
                <div
                  ...
                  onClick={() => onResolve(i.id, a.action_id)}
                >
                  {a.label}
                </div>
              ))}
