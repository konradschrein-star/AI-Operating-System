/**
 * vm-keys.ts — text → RFB keysym events. PURE: no DOM, no fetch, no logging.
 *
 * This is the half of "Type keys" that can be unit-tested without a browser.
 * It never sees a DOM and never touches console: the strings that pass through
 * here are Konrad's passwords, so the module has no side channel by construction.
 *
 * Rules (PLAN.md §1.1, measured by R1 in
 * docs/plan/aios-takeover-usable/research-keysym.md):
 *   - "\r\n" → ONE XK_Return (0xff0d); a lone "\n" or a lone "\r" → Return.
 *     R1 §2.5: keysymdef.lookup(0x0d) yields a "unicode CR" keysym, not Return,
 *     so CR must be normalised here and never fed to the table.
 ...
 *   - other C0 controls (U+0000–U+001F except \t \n \r) and U+007F are DROPPED:
 *     there is no keysym that types them and noVNC's lookup would produce a
 *     Unicode control keysym x11vnc cannot map to anything useful.
 *
 * The table below is noVNC 1.3.0's `core/input/keysymdef.js` (Debian
 * novnc 1:1.3.0-2, sha256 76f338d4…) minus its Latin-1 identity range, flattened
 * to [codepoint, keysym, …] pairs. Regenerate from that file; do not hand-edit.
 */
...
/* Flattened [codepoint, keysym] pairs, generated from noVNC keysymdef.js. */
const BMP_TABLE: readonly number[] = [
  ...
  0x30a5, 0x04a9, 0x30a6, 0x04b3, 0x30a7, 0x04aa, 0x30a8, 0x04b4, 0x30a9, 0x04ab, 0x30aa, 0x04b5,
...
const BMP_KEYSYMS: ReadonlyMap<number, number> = (() => {
  if (BMP_TABLE.length % 2 !== 0) {
    throw new Error(`vm-keys: BMP_TABLE has an odd length (${BMP_TABLE.length}); the generated table is corrupt`);
  }
  const m = new Map<number, number>();
  for (let i = 0; i < BMP_TABLE.length; i += 2) m.set(BMP_TABLE[i], BMP_TABLE[i + 1]);
  return m;
})();

/** How many table entries were loaded — for the check script, so a truncated
 *  regeneration cannot pass silently. noVNC 1.3.0 carries 659 non-Latin-1 rows. */
export const BMP_TABLE_SIZE = BMP_KEYSYMS.size;

/**
 * The keysym for one printable code point, mirroring noVNC's
 * `keysymdef.lookup(u)` (rfb.js feeds this straight into a KeyEvent):
 * Latin-1 → identity; table hit; else 0x01000000 | cp.
 */
export function keysymForCodePoint(cp: number): number {
  if (!Number.isInteger(cp) || cp < 0 || cp > 0x10ffff) {
    throw new RangeError(`vm-keys: ${cp} is not a Unicode code point`);
  }
  if (cp >= 0x20 && cp <= 0xff) return cp;
  const hit = BMP_KEYSYMS.get(cp);
  if (hit !== undefined) return hit;
  return 0x01000000 | cp;
}

/** True for a control character that has no typeable keysym and is dropped. */
function isDroppedControl(cp: number): boolean {
  return (cp < 0x20 && cp !== 0x09 && cp !== 0x0a && cp !== 0x0d) || cp === 0x7f;
}

/**
 * Text → ordered key events. Each event is ONE key press (the caller sends
 * down+up). Never throws on any string; never inspects or retains the text.
 */
export function textToKeyEvents(text: string): KeyEvent[] {
  ...
  for (const ch of text) {
    ...
    if (cp === undefined) continue; // unreachable: `for…of` yields whole code points
    if (cp === 0x0d) {
      out.push({ keysym: XK_RETURN });
      prevWasCR = true;
      continue;
    }
    if (cp === 0x0a) {
      if (!prevWasCR) out.push({ keysym: XK_RETURN }); // "\r\n" already produced its Return
      prevWasCR = false;
      continue;
    }
    prevWasCR = false;
    if (cp === 0x09) {
      out.push({ keysym: XK_TAB });
      continue;
    }
    if (isDroppedControl(cp)) continue;
    out.push({ keysym: keysymForCodePoint(cp) });
  }
  return out;
}

/** The count Send shows before typing starts ("0/120") — same rules, no array. */
export function countKeyEvents(text: string): number {
  return textToKeyEvents(text).length;
}
